...
interface decentralizedStorage{
    function addProject(address _contractAddress, string memory _name, string memory _refCode) external;
    function validateRefCode(string memory _refCode) external view returns(bool);
    function validateRouter(address _router) external view returns(bool);
    function setRefCodeUsed(string memory _refCode, address _token) external payable;
}

contract CUSTOMDIVIDENDTOKENDAPP is Ownable {

    address public storageMint;
    uint256 public oneNative = 1 * (10**18);
    uint256 public maxRewardFee = 10;
    uint256 public maxLiqFee = 10;
    uint256 public numberOfTokensCreated;
    uint256 public creationFees = 100000000000000;

    constructor(address _storage){
        storageMint = _storage;
    }

    function CreateCustomToken(address _rewardToken, string memory _tokenName, string memory _tokenSymbol, uint8 _decimals, uint256 _totalSupply, uint8 _rewardFee, uint8 _liqFee, address _router, string memory _refCode) public payable {
        require(maxLiqFee >= _liqFee,"Fee error");
        require(maxRewardFee >= _rewardFee,"Fee error");
        require(_rewardToken != address(0), "rewardToken cannot be zero addr");
        require(decentralizedStorage(storageMint).validateRefCode(_refCode), "invalid code");
        require(decentralizedStorage(storageMint).validateRouter(_router), "invalid router");
        require(msg.value >= creationFees, "msg.value must be >= creationFees!");

        address TokenCreation = address (new DxNativeDividendToken(_rewardToken, _tokenName, _tokenSymbol, _decimals,_totalSupply,_rewardFee,_liqFee,_router, IUniswapV2Router02(_router).WHT()));
        Ownable(TokenCreation).transferOwnership(msg.sender);

        decentralizedStorage(storageMint).addProject(TokenCreation, "dividendContractNativeReward", _refCode);
        numberOfTokensCreated++;

        decentralizedStorage(storageMint).setRefCodeUsed{value:msg.value}(_refCode,TokenCreation);
    }

    function changeStorageContract(address _storage) external onlyOwner {
        storageMint = _storage;
    }

    function updateFees(uint256 newfees) public onlyOwner {

        require(newfees >= 0, "invalid fees amount");

        creationFees = newfees;
